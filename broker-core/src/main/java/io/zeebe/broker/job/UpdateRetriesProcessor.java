...
public class UpdateRetriesProcessor implements CommandProcessor<JobRecord> {
  ...
  private static final String ERROR_MSG_NEGATIVE_RETRIES =
      "Expected retries to be greater than 0, but received %d";
  ...
  public void onCommand(TypedRecord<JobRecord> command, CommandControl<JobRecord> commandControl) {
    ...
    if (retries > 0) {
      ...
      if (updatedJob != null) {
        ...
        commandControl.reject(RejectionType.NOT_FOUND, String.format(ERROR_MSG_JOB_NOT_FOUND, key));
      }
    } else {
      commandControl.reject(
          RejectionType.INVALID_ARGUMENT, String.format(ERROR_MSG_NEGATIVE_RETRIES, retries));
    }
  }
}
